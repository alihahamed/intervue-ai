...
function CodeInterface({ onSubmit, onClose, isOpen }) {
  ...
  const instruction = useMemo(() => {
    ...
    const actualInstructions = recentMessages.find(m => m.text.length > 50 || recentMessages[0])
    console.log("Actual ins", actualInstructions)
      ...
      >
        ...
        <div className="hidden md:flex flex-col w-72 bg-zinc-900/30 border-r border-white/5 p-6">
          <div className="flex items-center gap-2 mb-8">
            ...
            <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-[0.2em]">Challenge</span>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 bg-zinc-900/20">
            <div className="flex flex-col md:hidden"> {/* Mobile Instruction Label */}
               <span className="text-[9px] font-bold text-emerald-500 uppercase mb-1">Current Task</span>
               <p className="text-zinc-400 text-[11px] line-clamp-1 italic">"{instruction}"</p>
            </div>
            ...
            >
              <span className="text-[10px] uppercase font-bold tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Exit</span>
          ...
          <div className="flex-1 relative">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-8 md:w-12 border-r border-white/[0.02] flex flex-col items-center pt-8 pointer-events-none">
                {[...Array(10)].map((_, i) => (
                  <span key={i} className="text-[10px] text-zinc-800 font-mono mb-4">{i + 1}</span>
                ))}
            </div>
          </div>
          ...
          <div className="p-4 md:p-6 bg-zinc-900/40 border-t border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-4">
               ...
               <span className="hidden sm:inline text-[10px] text-zinc-600 font-mono">UTF-8 // JavaScript</span>
            </div>
            ...
            >
              <span className="text-xs font-bold uppercase tracking-tighter">Execute Solution</span>
